...
async def _delete_part_by_id(
    ...
    """Internal function to delete a part by _id - ATOMIC OPERATION (same pattern as OCR delete)"""
    ...
    if isinstance(record["_id"], ObjectId):
        result = await db.part_records.delete_one({"_id": record["_id"]})
    else:
        result = await db.part_records.delete_one({"_id": record_id})
